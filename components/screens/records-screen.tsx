        ...
        {filteredRecords.length === 0 ? (
          ...
          <div className="space-y-3">
            {filteredRecords.map((record) => (
              <div
                key={record.id}
                className={`relative overflow-hidden bg-white/60 backdrop-blur-xl border border-white/60 shadow-[0_8px_32px_0_rgba(31,38,135,0.05)] rounded-2xl p-4 transition-all hover:scale-[1.02] ${
                ...
                {longPressedRecord === record.id && (
                  <div className="absolute inset-0 z-50 flex items-center justify-center bg-red-100/90 backdrop-blur-md rounded-2xl">
                    <button
                      ...
                      className="flex items-center gap-2 bg-red-500 text-white px-6 py-3 rounded-xl font-semibold text-sm shadow-lg hover:bg-red-600 transition-colors"
                    >
