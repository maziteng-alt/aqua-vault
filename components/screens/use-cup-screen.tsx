    ...
    <div className="flex flex-col gap-6 pb-4">
      ...
      <div className="bright-card p-5">
        ...
        <div className="relative mb-4">
          <div className="flex justify-between items-end gap-1 overflow-x-auto px-4">
            {Array.from({ length: 19 }, (_, index) => {
              const value = 50 + index * 25 // 50ml 到 500ml，19个柱子，间隔25ml
              const heightIndex = Math.min(index, 18)
              const heightClass = ['h-4', 'h-5', 'h-6', 'h-7', 'h-8', 'h-9', 'h-10', 'h-11', 'h-12', 'h-13', 'h-14', 'h-15', 'h-16', 'h-17', 'h-18', 'h-19', 'h-20', 'h-21', 'h-22'][heightIndex] || 'h-4'
              const isActive = Math.abs(volume - value) <= 15 // 选择最近的柱子
              return (
